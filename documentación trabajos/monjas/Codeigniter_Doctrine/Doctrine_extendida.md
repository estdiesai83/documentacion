...
6. Ahora puede utilizar doctrine en sus controladores para recuperar informacion, para actualizar, insertar,....
   ...
   Ejemplo de codigo, para utilizar los controladores
   ...
   class C_Doctrine extends \CodeIgniter\Controller
   {
       /**
        * Updates DB schema
        */
           ...
           foreach (scandir($modulePath) as $module){
               if(is_dir($modulePath."/".$module."/Entities")){
                   foreach (scandir($modulePath."/".$module."/Entities") as $moduleName){
                       if ($moduleName != "." && $moduleName != "..") $entities[] = $em->getClassMetadata(str_replace(".php", "",$module."\Entities\\".$moduleName));
                   }
               }
           }
           return $entities;
...
7. Ejemplo para obtener todos los casos de la tabla Test
...
Todos los comandos se ejecutan de la misma manera:

```bash
php doctrine command [options] [arguments]
Ej: php doctrine dbal:import /var/www/html/websocket/empresas.sql 
```
...
1. ###### list: 
   ...
   Muestra un listado de los diferentes comandos de doctrine y su correspondiente versión.

   ```bash
   Ej: php doctrine list
   ```

2. ###### dbal:import

   Importa archivo *.sql directamente en la base de datos.

   ```bash
   Ej: php doctrine dbal:import /var/www/html/websocket/empresas.sql 
   ```

3. ###### dbal:reserved-words

   Comprueba si la base de datos actual contiene identificadores reservados.

4. ###### dbal:run-sql

   Ejecuta SQL arbitrario directamente desde la línea de comando

   ```bash
   Ej: php doctrine dbal:run-sql 'select *from pruebas_code.test'
...
5. ###### orm:clear-cache:metadata

   Borre toda la caché de metadatos de los distintos controladores de caché.

6. ###### orm:clear-cache:query

   Borre todo el caché de consultas de los distintos controladores de caché

7. ###### orm:clear-cache:result

   Borre la caché de resultados de los distintos controladores de caché.

8. ###### orm:convert-d1-schema

   Convierte el esquema de Doctrine 1.X en un esquema de Doctrine 2.X.

9. ###### orm:convert-mapping

   Convierta la información de mapeo entre formatos compatibles.

10. ###### orm:ensure-production-settings

    Verifica que Doctrine esté configurado correctamente para un entorno de producción.

    En mi caso, al ejecutarlo, me muestra:

    ```bash
    php doctrine orm:ensure-production-settings
                                                                                                                            
    [ERROR] Query Cache uses a non-persistent cache driver, Doctrine\Common\Cache\ArrayCache.          
    ```

    Esto significa que no hay una caché persistente configurada para consultas de Doctrine. Doctrine usa una caché para convertir consultas DQL a SQL, por lo que en un entorno de producción tiene sentido almacenar esto en caché y no tener que hacer el mismo trabajo cada vez.

    Para ello:

    Prueba 1: Diferentes testeos demuestran que la APC cachė es uno de los mejores sistemas de cacheo.
    ...
    Una vez instalado añadir en el archivo php.ini al final las lineas:
    ...
    Aun asi: me sigue el mismo error en la consola de php doctrine

11. ###### orm:info:

    Muestra informacion básica sobre las entidades mapeadas.

    ```
    php doctrine orm:info
    
    EJ:Found 3 mapped entities:
...
12. ###### orm:generate-entities

    Genera clases de entidad y códigos auxiliares de métodos a partir de su información de mapeo.
...
13. ###### orm:generate-proxies 

    Genera clases de proxy para clases de entidad.

    ```
    php doctrine orm:generate-proxies Models/Proxies/
    ```

    

14. ###### orm:generate-repositories

    Genere clases de repositorio a partir de su información de mapeo.

15. ###### orm:run-dql

    Ejecuta DQL arbitrario directamente desde la línea de comandos.

    ```
    php doctrine orm:run-dql "SELECT a FROM Entities\Test a"
    ...
    array(3) {
      ...
      object(stdClass)#209 (3) {
        ["__CLASS__"]=>
        string(13) "Entities\Test"
        ["idTest:Entities\Test:private"]=>
        int(1)
        ["text:Entities\Test:private"]=>
        string(6) "texto1"
      }
      [1]=>
...
16. `orm:schema-tool:create` 

    Procesa el esquema y lo crea directamente en EntityManager Storage Connection o genera la salida SQL. Es decir ejecuta las entidades y crea las estructuras en la base de datos correspondiente.


Nota: La consola también admite la finalización automática, por ejemplo, en lugar de `orm:clear-cache:query`usar solo `o:c:q`.
...
$user = new \Entities\Empresa;//sin \ me da error
$user->setEmpresa('Mr.Right');
$user->setSector('Sector1');
$em->persist($user);
$em->flush();
