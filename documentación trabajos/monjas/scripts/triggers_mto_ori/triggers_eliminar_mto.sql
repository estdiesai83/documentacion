...
/*!50003 CREATE*/ /*!50003 TRIGGER trigger_eliminar_mto_elemento
   AFTER DELETE ON mto_elemento_trb
   FOR EACH ROW
   BEGIN
...
/*!50003 CREATE*/ /*!50003 TRIGGER trigger_eliminar_mto_operacion
   ...
   -- Mira cuantos casos hay en mto_relacion_modo con el codigo_operacion igual al
   -- de la operacion eliminada, si no hay ninguno eliminar sesion
   SELECT codigo_modo
   INTO cod_modo
   FROM mto_relacion_modo WHERE codigo_operacion = OLD.id;

   DELETE FROM mto_relacion_modo WHERE codigo_operacion = OLD.id;

   -- NOTA: no se puede poner el codigo de abajo porque da fallo si se elimina
   -- un modo desde mysql por que se hace dos veces
   -- SELECT COUNT(codigo_modo)
   -- INTO num_ope
   -- FROM mto_relacion_modo WHERE codigo_modo = cod_modo;

   -- IF num_ope = 0 THEN
   --    DELETE FROM mto_modo WHERE id = cod_modo;
   -- END IF;
