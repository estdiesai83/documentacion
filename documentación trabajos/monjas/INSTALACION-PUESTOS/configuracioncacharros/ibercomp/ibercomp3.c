...
{
	g_printf("\nescribirpuertoserie----tramaenviar:[%d][%d][%d][%d][%d]\n",tramaenviar[0],tramaenviar[1],tramaenviar[2],tramaenviar[3],tramaenviar[4]);
...
//
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
gint resetearcontador_ibercomp(GList *datalista)
{
	GList *listaprogramar=datalista;
	//
	gint respuesta=-1, estado=-1, idsemaforo=-1;
	...
	{		
		...
		velocidad13=atoi(g_list_nth_data(listaprogramar,13)); 	//contador

		g_printf("\nCONTADOR=%d----------RESETEAR------------\n",velocidad13);
		valor13_ida=velocidad13;
		
		//////////////////////////////////////////////////
		// enviar:  <comando> <163> <contador> <crc>	//
		// recibir: <crc>								//
		//////////////////////////////////////////////////
		if(eco==1)
			tamtramavuelta=5;
		...
		tramaida[0] = comando;
		tramaida[1] = 163;
		tramaida[2] = valor13_ida;
		tramaida[3] = tramaida[0] + tramaida[1] + tramaida[2];
		tramaida[4] = 0;
		
		g_printf("\nfuncionesespeciales----resetear contador ibercomp----puerto---->%s---->tramaida:[%d][%d][%d][%d][%d]\n",puerto,tramaida[0],tramaida[1],tramaida[2],tramaida[3],tramaida[4]);
					
		estado = FEF_abrirpuertoserie_funcionesespeciales_libgsksoa(idpuerto,&fdpuerto);	
		if(fdpuerto==-1)
		{
			FEF_logespeciales_funcionesespeciales_libgsksoa (0,comando,0,"ibercomp_ed","resetear",3,idpuerto,NULL);			
		}
		else
		{
			estado=FEF_configurarpuertoserie_funcionesespeciales_libgsksoa(fdpuerto,velocidad,paridad,bitstop,tipolectura,tipoenviobits,bitsdatos);	
			if(estado==-1)
			{
				FEF_logespeciales_funcionesespeciales_libgsksoa (0,comando,0,"ibercomp_ed","resetear",4,idpuerto,NULL);			
				FEF_cerrarpuertoserie_funcionesespeciales_libgsksoa(fdpuerto);
			}
		}
		...
		if( (estado!=-1) && (fdpuerto>=0) )
		{
				...
				if (respuesta == -1)
				{
						FEF_logespeciales_funcionesespeciales_libgsksoa (0,comando,0,"ibercomp_ed","resetear",5,idpuerto,NULL);			
				}
				else
				{
						if ( tiempolectura >= 1000000 )						
						...
						if (tramavuelta == NULL)
						{
							FEF_logespeciales_funcionesespeciales_libgsksoa (0,comando,0,"ibercomp_ed","resetear",6,idpuerto,NULL);			
							usleep(tiempodesbloqueo);
						}
						else
						{
							haydato_hayeco_ok=TRUE;
							crccorrecto = tramaida[0]+tramaida[1]+tramaida[2];
							if(eco==1)
							{
								crcrecibido = tramavuelta [4]; //crc del eco
								if(tramaida[3] != tramavuelta[3])
								{
									FEF_logespeciales_funcionesespeciales_libgsksoa (0,comando,0,"ibercomp_ed","resetear",7,idpuerto,NULL);			
									haydato_hayeco_ok=FALSE;
									usleep(tiempodesbloqueo);
								}
								else
								{
									if( ( tramaida[3] == tramavuelta[3] ) && 
										( tramavuelta[4] == 0 ) )
									{
										FEF_logespeciales_funcionesespeciales_libgsksoa (0,comando,0,"ibercomp_ed","resetera",8,idpuerto,NULL);			
										haydato_hayeco_ok=FALSE;
										usleep(tiempodesbloqueo);
									}
								}
								g_printf("\n>>>comando enviado:[%d][%d][%d][%d]<-------->TRAMA VUELTA=recibido:[%d]\n",tramaida[0],tramaida[1],tramaida[2],tramaida[3],tramavuelta[4]);
							}
							else
							{
								crcrecibido = tramavuelta [0];	
								g_printf("\n>>>comando enviado:[%d][%d][%d][%d]<-------->TRAMA VUELTA=recibido:[%d]\n",tramaida[0],tramaida[1],tramaida[2],tramaida[3],tramavuelta[0]);
							}
							...
							{							
								FEF_logespeciales_funcionesespeciales_libgsksoa (0,comando,0,"ibercomp_ed","resetear",9,idpuerto,NULL);			
								actualizar_pcb=FALSE;
								usleep(tiempodesbloqueo);
							}							
						}							
				...
				if(actualizar_pcb)
				{
					g_printf("\n>OPERACION IBERCOMP RESETEAR CONTADOR OK comando:[%d]<\n",comando);	
					return 1;
				}
		}		
	}
	
	g_printf("\n>OPERACION IBERCOMP RESETEAR FIN comando:[%d]<\n",comando);					
	return -1;
}
//////////////////
gint valorrespaldocontador_ibercomp(GList *datalista)
{
	GList *listaprogramar=datalista;
	//
	gint respuesta=-1, estado=-1, idsemaforo=-1;
	...
	gint tamtramavuelta = 0;	
	guchar tramaida[6]={0}, *tramavuelta = NULL;	
	guchar tramaida2[6]={0};	
	guchar tramaida3[6]={0};	
	guchar tramaida4[6]={0};	
	gint fdpuerto;
	guchar crccorrecto = 0, crcrecibido = 0, comandoenviado=0, comandorecibido=0;
	gboolean haydato_hayeco_ok=FALSE;
	gboolean actualizar_pcb=FALSE;	
	...
	{		
		...
		velocidad13=atoi(g_list_nth_data(listaprogramar,13)); 	//contador
		valor13=atoi(g_list_nth_data(listaprogramar,14)); 	//valor
		g_printf("\nCONTADOR(REGISTROS)=%d------>VALOR A PONER=%d\n",velocidad13,valor13);
		valor13_ida=valor13;
		
		//////////////////////////////////////////////////
		// enviar:  <comando> <196> CONTADOR1=<17><18><19><20>.ETC...............= <valor> <crc>	//
		// recibir: <crc>								//
		//////////////////////////////////////////////////
		if(eco==1)
			tamtramavuelta=6;
		...
		tramaida[0] = comando;	
		//CADA CONTADOR 4 DIRECCIONES DIFERENTES//
		//poner a maximo valor = FF FF FF FF = 4.294.967.295//
		//
		tramaida[1] = 196;
		tramaida[2] = 20;	//reg=17-18-19-20  -----> contador 0
		tramaida[3] = valor13_ida;
		tramaida[4] = tramaida[0] + tramaida[1] + tramaida[2] + tramaida[3];
		tramaida[5] = 0;
		//
		/*
		tramaida2[1] = 196;
		tramaida2[2] = 18;	//reg=18
		tramaida2[3] = valor13_ida;
		tramaida2[4] = tramaida2[0] + tramaida2[1] + tramaida2[2] + tramaida2[3];
		tramaida2[5] = 0;
		//
		tramaida3[1] = 196;
		tramaida3[2] = 19;	//reg=19
		tramaida3[3] = valor13_ida;
		tramaida3[4] = tramaida3[0] + tramaida3[1] + tramaida3[2] + tramaida3[3];
		tramaida3[5] = 0;
		//
		tramaida4[1] = 196;
		tramaida4[2] = 20;	//reg=20
		tramaida4[3] = valor13_ida;
		tramaida4[4] = tramaida4[0] + tramaida4[1] + tramaida4[2] + tramaida4[3];
		tramaida4[5] = 0;
		//
		*/		
		//g_printf("\nfuncionesespeciales----programar ibercomp----puerto---->%s---->tramaida:[%d][%d][%d][%d][%d]\n",puerto,tramaida[0],tramaida[1],tramaida[2],tramaida[3],tramaida[4]);
		...
		if( (estado!=-1) && (fdpuerto>=0) )
		{
				...
				//
				sleep(1);
				//respuesta = FEF_escribirpuertoserie_funcionesespeciales_libgsksoa(fdpuerto,tramaida3,5);
				//
				//sleep(1);
				//respuesta = FEF_escribirpuertoserie_funcionesespeciales_libgsksoa(fdpuerto,tramaida4,5);
				//
				if (respuesta == -1)
				{
						FEF_logespeciales_funcionesespeciales_libgsksoa (0,comando,0,"ibercomp_fe","programar",5,idpuerto,NULL);			
...
//////////////////
gint vervalorcontador_ibercomp(GList *datalista)
{
	GList *listaprogramar=datalista;
	//
	gint respuesta=-1, estado=-1, idsemaforo=-1;
	...
	gint valorparametro_c3=0;	
	gint valorparametro_c2=0;	
	gint valorparametro_c1=0;	
	gint valorparametro_c0=0;	
	gdouble valorpasar=0;
	gdouble resultadototal=0;
	gchar resultadocontador[15]={0};
	...
	{		
		...
		g_printf("\nCONTADOR=%d----------VER VALOR------------\n",velocidad13);
		valor13_ida=velocidad13;
		
		//////////////////////////////////////////////////
		// enviar:  <comando> <135> <contador> <crc>	//
		// recibir: cont+cont+cont+cont <crc>								//
		//////////////////////////////////////////////////
		if(eco==1)
			tamtramavuelta=9;
		else
		...
		g_printf("\nfuncionesespeciales----ver valor contador ibercomp----puerto---->%s---->tramaida:[%d][%d][%d][%d][%d]\n",puerto,tramaida[0],tramaida[1],tramaida[2],tramaida[3],tramaida[4]);
		...
		if( (estado!=-1) && (fdpuerto>=0) )
		{
				...
				else
				{
						...
						else
						{
							...
							if(eco==1)
							{
								crccorrecto = crccorrecto + tramavuelta [4] + tramavuelta [5] + tramavuelta [6] + tramavuelta [7];
								crcrecibido = tramavuelta [8];
								valorparametro_c3 = (gint) tramavuelta [4];		//
								valorparametro_c2 = (gint) tramavuelta [5];		//
								valorparametro_c1 = (gint) tramavuelta [6];		//
								valorparametro_c0 = (gint) tramavuelta [7];		//
								...
								{
									if( ( tramaida[3] == tramavuelta[3] ) && 
										( tramavuelta[4] == 0 ) && ( tramavuelta[5] == 0 ) && ( tramavuelta[6] == 0 ) && ( tramavuelta[7] == 0 ) && ( tramavuelta[8] == 0 ) ) //																			
									{
										FEF_logespeciales_funcionesespeciales_libgsksoa (0,comando,0,"ibercomp_ed","resetera",8,idpuerto,NULL);			
										haydato_hayeco_ok=FALSE;
										usleep(tiempodesbloqueo);
							...
							{
								crccorrecto = crccorrecto + tramavuelta [0] + tramavuelta [1] + tramavuelta [2] + tramavuelta [3];
								crcrecibido = tramavuelta [4];
								valorparametro_c3 = (gint) tramavuelta [0];		//
								valorparametro_c2 = (gint) tramavuelta [1];		//
								valorparametro_c1 = (gint) tramavuelta [2];		//
								valorparametro_c0 = (gint) tramavuelta [3];		//
								//
								g_printf("\n>>>comando enviado:[%d][%d][%d][%d]<-------->TRAMA VUELTA=recibido:[%d]\n",tramaida[0],tramaida[1],tramaida[2],tramaida[3],tramavuelta[0]);
							}
				...
				//{
					...
					valorpasar=valorparametro_c0;
					resultadototal=resultadototal+valorpasar;
					valorpasar=valorparametro_c1;
					resultadototal=resultadototal+(valorpasar*256);
					valorpasar=valorparametro_c2;
					resultadototal=resultadototal+(valorpasar*256*256);
					valorpasar=valorparametro_c3;
					resultadototal=resultadototal+(valorpasar*256*256*256);
					sprintf(resultadocontador,"%.0f",resultadototal);
					//
					g_printf("\n>OPERACION IBERCOMP VALOR CONTADOR comando:[%d]--RESULTADO---->[%s]<-------<\n",comando,resultadocontador);	
					return 1;
				//}
		}		
	}
	
	g_printf("\n>OPERACION IBERCOMP VER VALOR CONTADOR FIN comando:[%d]<\n",comando);					
	return -1;
}
//////////////////
gint vervalorregistro_ibercomp(GList *datalista)
{
	GList *listaprogramar=datalista;
	//
	gint respuesta=-1, estado=-1, idsemaforo=-1;
	...
	{		
		...
		g_printf("\nREGISTRO=%d----------VER VALOR------------\n",velocidad13);
		valor13_ida=velocidad13;
		
		//////////////////////////////////////////////////
		// enviar:  <comando> <228> <registro> <crc>	//
		// recibir: <valor><crc>								//
		//////////////////////////////////////////////////
		if(eco==1)
			tamtramavuelta=6;
		...
		g_printf("\nfuncionesespeciales----ver valor registro ibercomp----puerto---->%s---->tramaida:[%d][%d][%d][%d][%d]\n",puerto,tramaida[0],tramaida[1],tramaida[2],tramaida[3],tramaida[4]);
					
		estado = FEF_abrirpuertoserie_funcionesespeciales_libgsksoa(idpuerto,&fdpuerto);	
		if(fdpuerto==-1)
		{
			FEF_logespeciales_funcionesespeciales_libgsksoa (0,comando,0,"ibercomp_ed","verregistro",3,idpuerto,NULL);			
		}
		else
		{
			estado=FEF_configurarpuertoserie_funcionesespeciales_libgsksoa(fdpuerto,velocidad,paridad,bitstop,tipolectura,tipoenviobits,bitsdatos);	
			if(estado==-1)
			{
				FEF_logespeciales_funcionesespeciales_libgsksoa (0,comando,0,"ibercomp_ed","verregistro",4,idpuerto,NULL);			
				FEF_cerrarpuertoserie_funcionesespeciales_libgsksoa(fdpuerto);
			}
		}
		...
		if( (estado!=-1) && (fdpuerto>=0) )
		{
				...
				if (respuesta == -1)
				{
						FEF_logespeciales_funcionesespeciales_libgsksoa (0,comando,0,"ibercomp_ed","verregistro",5,idpuerto,NULL);			
				}
				else
				{
						if ( tiempolectura >= 1000000 )						
						...
						if (tramavuelta == NULL)
						{
							FEF_logespeciales_funcionesespeciales_libgsksoa (0,comando,0,"ibercomp_ed","verregistro",6,idpuerto,NULL);			
							usleep(tiempodesbloqueo);
						}
						else
						{
							...
							if(eco==1)
							{
								crccorrecto = crccorrecto + tramavuelta [4];
								crcrecibido = tramavuelta [5];
								valorparametro_c0 = (gint) tramavuelta [4];		//
																												
								//
								if(tramaida[3] != tramavuelta[3])
								{
									FEF_logespeciales_funcionesespeciales_libgsksoa (0,comando,0,"ibercomp_ed","verregistro",7,idpuerto,NULL);			
									haydato_hayeco_ok=FALSE;
									usleep(tiempodesbloqueo);
								}
								else
								{
									if( ( tramaida[3] == tramavuelta[3] ) && 
										( tramavuelta[4] == 0 ) && ( tramavuelta[5] == 0 ) ) //																			
									{
										FEF_logespeciales_funcionesespeciales_libgsksoa (0,comando,0,"ibercomp_ed","verregistro",8,idpuerto,NULL);			
										haydato_hayeco_ok=FALSE;
										usleep(tiempodesbloqueo);
									}
								}
								g_printf("\n>>>recibido:[%d][%d][%d][%d]<-------->TRAMA VUELTA=recibido:[%d][%d]\n",tramavuelta[0],tramavuelta[1],tramavuelta[2],tramavuelta[3],tramavuelta[4],tramavuelta[5]);
							}
							else
							{
								crccorrecto = crccorrecto + tramavuelta [0];
								crcrecibido = tramavuelta [1];
								valorparametro_c0 = (gint) tramavuelta [0];		//
								//
								g_printf("\n>>>comando enviado:[%d][%d][%d][%d]<-------->TRAMA VUELTA=recibido:[%d]\n",tramaida[0],tramaida[1],tramaida[2],tramaida[3],tramavuelta[0]);
							}
							...
							{							
								FEF_logespeciales_funcionesespeciales_libgsksoa (0,comando,0,"ibercomp_ed","verregistro",9,idpuerto,NULL);			
								actualizar_pcb=FALSE;
								usleep(tiempodesbloqueo);
							}							
						}							
				...
				//{
					...
					sprintf(resultadocontador,"%.0f",resultadototal);
					//
					g_printf("\n>OPERACION IBERCOMP VALOR REGISTRO comando:[%d]--RESULTADO---->[%s]<-------<\n",comando,resultadocontador);	
					return 1;
				//}
		}		
	}
	
	g_printf("\n>OPERACION IBERCOMP VER VALOR REGISTRO FIN comando:[%d]<\n",comando);					
	return -1;
}
//////////////////
...
{	
	...
	//recibe 4/5/6 argumentos: tipo operacion(0,1)-comando-puerto(0,1,..)-[velocidad]-eco(1,2)//
	...
	if(argv[1]!=NULL)
	{
		...
		switch(operacion)
		{
			...
			if(argv[2]!=NULL && argv[3]!=NULL && argv[4]!=NULL && argv[5]!=NULL)
			{
				...
				listaoperacion=g_list_append(listaoperacion,g_strconcat("0",NULL));	
				listaoperacion=g_list_append(listaoperacion,g_strconcat("0",NULL));		
				listaoperacion=g_list_append(listaoperacion,g_strconcat(argv[4],NULL)); //contador

				resultado=resetearcontador_ibercomp(listaoperacion);
	
				if(resultado!=1)
				{
					g_print( "\n>>>>>>OPERACION IBERCOMP - EJECUTADA........?<<<<<\n" );	
				}				
			}
			else
			{
				g_print( "\n>>>>>>OPERACIONES IBERCOMP - ERROR EN ARGUMENTOS: operacion 2<<<<<\n" );		
				g_print( "\n      ARGUMENTOS : operacion  comando  puerto  contador  eco\n" );
				g_print( "                     ej.: 2 10 0 5 1\n" );
			}
			break;
			case 3:
			if(argv[2]!=NULL && argv[3]!=NULL && argv[4]!=NULL && argv[5]!=NULL && argv[6]!=NULL)
			{
				...
				listaoperacion=g_list_append(listaoperacion,g_strconcat("0",NULL));		
				listaoperacion=g_list_append(listaoperacion,g_strconcat(argv[4],NULL)); //contador
				listaoperacion=g_list_append(listaoperacion,g_strconcat(argv[5],NULL)); //valor

				resultado=valorrespaldocontador_ibercomp(listaoperacion);
	
				if(resultado!=1)
				{
					g_print( "\n>>>>>>OPERACION IBERCOMP - ERROR - EJECUTADA....?<<<<<\n" );	
				}				
			}
			else
			{
				g_print( "\n>>>>>>OPERACIONES IBERCOMP - ERROR EN ARGUMENTOS: operacion 3<<<<<\n" );		
				g_print( "\n      ARGUMENTOS : operacion  comando  puerto  contador  valor   eco\n" );
				g_print( "                     ej.: 2 10 0 5 1000 1\n" );
			}
			break;
			case 4:
			if(argv[2]!=NULL && argv[3]!=NULL && argv[4]!=NULL && argv[5]!=NULL)
			{
				...
				resultado=vervalorcontador_ibercomp(listaoperacion);
			...
			{
				g_print( "\n>>>>>>OPERACIONES IBERCOMP - ERROR EN ARGUMENTOS: operacion 4<<<<<\n" );		
				g_print( "\n      ARGUMENTOS : operacion  comando  puerto  contador  eco\n" );
				g_print( "                     ej.: 4 2 0 3 1\n" );
			}
			break;
			case 5:
			if(argv[2]!=NULL && argv[3]!=NULL && argv[4]!=NULL && argv[5]!=NULL)
			{
				...
				listaoperacion=g_list_append(listaoperacion,g_strconcat("0",NULL));	
				listaoperacion=g_list_append(listaoperacion,g_strconcat("0",NULL));		
				listaoperacion=g_list_append(listaoperacion,g_strconcat(argv[4],NULL)); //registro

				resultado=vervalorregistro_ibercomp(listaoperacion);
			...
			{
				g_print( "\n>>>>>>OPERACIONES IBERCOMP - ERROR EN ARGUMENTOS: operacion 5<<<<<\n" );		
				g_print( "\n      ARGUMENTOS : operacion  comando  puerto  registro  eco\n" );
				g_print( "                     ej.: 5 2 0 16 1\n" );
			}
			break;
			default:
	...
	else
	{
		...
		g_print( "\n      ARGUMENTO 1= operacion(0,1,2,3,4,5)\n" );				
