...
static devfs_handle_t snx_devfs_handle = NULL;
#endif

static struct snx_lp_struct snx_lp_table[SNX_LP_NO];

static unsigned int snx_lp_count = 0;
...
static int snx_lp_init(void)
{
	...
	if (register_chrdev(SNX_PAL_MAJOR, "lx", &snx_lp_fops)) 
	{
		printk("SNX Error: lp unable to get major %d.\n", SNX_PAL_MAJOR);
		return -EIO;
	}
