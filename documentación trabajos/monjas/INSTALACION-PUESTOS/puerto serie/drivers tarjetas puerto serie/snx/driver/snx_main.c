/*
 *                                                                                      	
 *                Multi-I/O Board Device Driver                    
 *                                                                                                                
 *					 		
 *	
 *	Driver for Multi-I/O Board device driver						
...
static int sunix_pci_board_probe(void)
{
    ...
    while (tablecnt < sunix_pci_board_id_cnt) 
    {
        ...
        if (sub_device_id == 0)
        {
            printk("SNX Error: Board (bus:%d device:%d), in configuration space,\n", pdev->bus->number, PCI_SLOT(pdev->devfn));
        ...
        else
        {
            ...
            if (status != 0)
            { 
                printk("SNX Error: Board Enable Fail !\n\n");
                status = -ENXIO;
                return status;
            }
        }
        ...
        {
            printk("\n");
            printk("SNX Error: Driver Module Support Four Boards In Maximum !\n\n");   
            status = -ENOSPC;
            return status;
        }
    ...
    while (tablecnt < sunix_pci_board_id_cnt) 
    {
        ...
        bar3_bass_add = pci_resource_start(pdev, 3);    
                 
        device_part_number = inb(bar3_bass_add + 5);       
    ...
    if (boardcnt == 0)
    {
        printk("SNX Info : No Multi-I/O Board Found !\n\n");
        status = -ENXIO;
        return status;
    }
	else
...
static int sunix_get_pci_board_conf(void)
{
    ...
    for (i = 0; i < SNX_BOARDS_MAX; i++)
    {
        ...
        if (sb->board_enum > 0)
        {
          ...
          if (sb->irq <= 0)
          {
          	printk("SNX Error: Board %s Series (bus:%d device:%d), in configuartion space, irq isn't valid !\n\n",
...
static int sunix_par_port_table_init(void)
{
	...
	for (i = 0; i < SNX_BOARDS_MAX; i++)
	{
		...
		if ((sb->board_enum > 0) && (sb->par_port > 0))
		{
			...
			for (j = 0; j < SNX_PAR_TOTAL_MAX; j++)
			{
				...
				if ((pp->bus_number == sb->bus_number) && (pp->dev_number == sb->dev_number) && (pp->board_enum	= sb->board_enum))
				{
					...
					{
						pp->snx_type		= SNX_PAR_PORT_SUN1888;
					}
					else if (pp->chip_flag == SUN1689_HWID) 
					{
						pp->snx_type		= SNX_PAR_PORT_SUN1689;
					}
					else if (pp->chip_flag == SUNMATX_HWID) 
					{
						pp->snx_type		= SNX_PAR_PORT_SUNMATX;
					}						
					else if (pp->chip_flag == SUN1999_HWID) 
					{
						pp->snx_type		= SNX_PAR_PORT_SUN1999;
					}				
					else
					{
						pp->snx_type		= SNX_PAR_PORT_UNKNOWN;
					}
	...
	for (i = 0; i < SNX_PAR_TOTAL_MAX; i++)
	{
		...
		if (pp->base > 0)
		{
			...
			release_region(pp->base + SNX_PAR_STD_ADDR_LENGTH, SNX_PAR_ETD_ADDR_LENGTH);

			if (pp->base_hi > 0)
			{
				release_region(pp->base_hi, SNX_PAR_ADDRESS_LENGTH);	

				release_region(pp->base_hi, SNX_PAR_STD_ADDR_LENGTH);
				release_region(pp->base_hi + SNX_PAR_STD_ADDR_LENGTH, SNX_PAR_ETD_ADDR_LENGTH);
			}
		}
	}
...
void sunix_debug(void)
{
	...
	for (j = 0; j < SNX_SER_TOTAL_MAX; j++)
	{
		...
		if (sp->port.iobase)
		{
			...
			printk(" irq          : %d\n",   sp->port.irq);
			printk(" vector       : x%4x\n", sp->port.vector);
			printk(" vector_mask  : x%4x\n", sp->port.vector_mask);
			printk(" chip_flag    : x%2x\n", sp->port.chip_flag);
			printk(" port_flag    : x%2x\n", sp->port.port_flag);				 
			printk("----------------------------\n");
		}
	}
	printk("============================\n");
	...
	printk("\n");
	printk("======== par  info ========\n");
	for (k = 0; k < SNX_PAR_TOTAL_MAX; k++)
	{
		pp = &sunix_par_table[k];
		if (pp->base)
		{
			printk(" number       : %d\n",   k);
			printk(" name         : %s\n",   pp->pb_info.board_name);
			printk(" base         : x%4x\n", (unsigned int)pp->base);
			printk(" base_hi      : x%4x\n", (unsigned int)pp->base_hi);			
			printk(" chip_flag    : x%2x\n", pp->chip_flag);
			printk(" port_flag    : x%2x\n", pp->port_flag);				 
			printk("----------------------------\n");
		}
...
int sunix_register_irq(void)
{
	...
	for (i = 0; i < SNX_BOARDS_MAX; i++)
	{
		...
		{
#if (LINUX_VERSION_CODE > KERNEL_VERSION(2,6,18))
			status = request_irq(sb->irq, sunix_interrupt, IRQF_SHARED, "snx", sb);
#else
            status = request_irq(sb->irq, sunix_interrupt, SA_SHIRQ, "snx", sb);
#endif	
		
			if (status) 
			{
	 			printk("SNX Error: Multi-I/O %s Board(bus:%d device:%d), request\n", sb->pb_info.board_name, sb->bus_number, sb->dev_number);	 			
...
static int __init snx_init(void)
{
	...
	printk("\n\n");
	printk("=======================  Device Driver Module Install  =========================\n");
	printk("\n");
	printk("SNX Info : Loading Multi-I/O Board Driver Module\n");
	printk("                                                       -- Date :    %s\n", SNX_DRIVER_DATE);
	printk("                                                       -- Version : %s\n\n", SNX_DRIVER_VERSION);
...
step1_fail:
	...
	printk("SNX Error: Couldn't Loading Multi-I/O Board Driver Module correctly,\n"); 
...
{
	printk("\n\n");
	printk("=======================  Device Driver Module Uninstall  =======================\n");
	printk("\n");
	...
	printk("SNX Info : Unload Multi-I/O Board Driver Module Done.\n");
	printk("================================================================================\n");
}
