...
static void print_info(struct snx_info *infop)
{
	...
	{
	    printf("\n================ Found %2d LINUX port , list informations ====================\n", infop->ser_count + infop->par_count);
	    if (infop->ser_status2 == 0)
		    printf("                                             LINUX driver ver -- %s\n\n", infop->driver_ver);
	    else
		    printf("\n");
	    ...
	    for (i = 0; i < SNX_SER_TOTAL_MAX; i++)
	    {
		    ...
		    printf("LINUX %s Series (bus:%d device:%2d) , base address = %4x,    irq = %2d\n\n",
	    ...
	    for (i = 0; i < SNX_PAR_TOTAL_MAX; i++)
	    {
		    ...
		    printf("LINUX %s Series (bus:%d device:%2d) , base addr = %4x, extd addr = %4x\n\n",
