          ...
          sb = &sunix_board_table[gb.board_id -1];
        	...
        	{	
        		switch(sb->gpio_chl_cnt)
					...
					{
						printk("WARNING : GPIO type is %d",sb->board_gpio_type);
					}	
					*/					
        	break;	
        }	
          ...
          {          	
        	...
        	bar3_base_Address = pci_resource_start(sb->pdev,3); 
					...
					switch(uart_type)
					{
						...
						case 1: // RS-422/485
						{
							if(AHDCstate && RS422state) //Auto 422/485
							{
								gb.uart_type = 3;
							}
							else if(AHDCstate && !RS422state) //RS-485
							{
								gb.uart_type = 2;
							}	
							else if(!AHDCstate && RS422state) //RS-422
							{
								gb.uart_type = 1;
							}	
							else // unknown case
						...
						case 2: // RS-232/422/485
						{
							...
							else if(!AHDCstate && !RS422state) //RS232
							{
								gb.uart_type = 0;		
							}
							else // unknown case
        ...
        case SNX_UART_SET_TYPE :
        {
          ...
          {          	
        	...
        	bar3_byte5 = inb(sb->bar_addr[3] + 5);
					...
					if(uart_type == 0)// RS-232 series
					{
						if(gb.uart_type == 0)
						{
							AHDCstate &= ~( 0x01 << ( ( gb.uart_num - 1 ) % 4 ) );
							RS422state &= ~ (0x10 << ( ( gb.uart_num - 1 ) % 4 ) );	//RS422 Enable
							RS422state &= ~ (0x01 << ( ( gb.uart_num - 1 ) % 4 ) );	//ACS
							GPIOstate |=( 0x01 << ( ( gb.uart_num - 1 ) % 4 ) );
							GPIOcontrol |=( 0x01 << ( ( gb.uart_num - 1 ) % 4 ) );	
					...
					else if (uart_type == 1)// RS-422/485 series
					{
						if(gb.uart_type == 3)// Auto RS-422/485 state
						...
						else if (gb.uart_type == 2)	// RS-485 state
						{
							AHDCstate |=( 0x01 << ( ( gb.uart_num - 1 ) % 4 ) );
							AHDCstate |=( 0x10 << ( ( gb.uart_num - 1 ) % 4 ) );						
							RS422state &= ~ (0x10 << ( ( gb.uart_num - 1 ) % 4 ) );							
						...
						else if (gb.uart_type == 1)	// RS-422 state
						{
							AHDCstate |=( 0x10 << ( ( gb.uart_num - 1 ) % 4 ) );	
							AHDCstate &= ~( 0x01 << ( ( gb.uart_num - 1 ) % 4 ) );					
							RS422state |= (0x10 << ( ( gb.uart_num - 1 ) % 4 ) );
					...
					else if (uart_type == 2)	// 3-in-1 series	
					{								
						if(gb.uart_type == 3)// Auto RS-422/485 state
						{
							AHDCstate |=( 0x01 << ( ( gb.uart_num - 1 ) % 4 ) );
						...
						else if (gb.uart_type == 0)	// RS-232 state	
						{
							AHDCstate &= ~( 0x01 << ( ( gb.uart_num - 1 ) % 4 ) );
							RS422state &= ~ (0x10 << ( ( gb.uart_num - 1 ) % 4 ) );	//RS422 Enable
							RS422state &= ~ (0x01 << ( ( gb.uart_num - 1 ) % 4 ) );	//ACS
