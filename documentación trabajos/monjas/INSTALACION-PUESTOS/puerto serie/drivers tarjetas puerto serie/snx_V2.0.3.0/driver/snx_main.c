...
void sunix_debug(void)
{
	...
	for (i = 0; i < SNX_BOARDS_MAX; i++)
	{
		...
		if (sb->board_enum != -1)
		{
			...
			printk(" oem_id       : %4x\n", sb->oem_id);
			printk(" uart_cnt     : %d\n", sb->uart_cnt);
			printk(" gpio_chl_cnt          : %d\n", sb->gpio_chl_cnt);
			printk(" board_uart_type       : %2x\n", sb->board_uart_type);
			printk(" board_gpio_card_type  : %2x\n", sb->board_gpio_card_type);
			printk(" board_gpio_type   		 : %2x\n", sb->board_gpio_type);
		}
	}
	printk("============================\n");
	printk("\n");
	...
	for (j = 0; j < SNX_SER_TOTAL_MAX; j++)
	{
		...
		if (sp->port.iobase)
		{
			...
			printk(" AHDC_State    : x%d\n", sp->port.AHDC_State);
			printk(" RS422_State    : x%d\n", sp->port.RS422_State);
			printk("----------------------------\n");
		}
	}
	printk("============================\n");
...
int sunix_register_irq(void)
{
	...
	for (i = 0; i < SNX_BOARDS_MAX; i++)
	{
      ...
      status = request_irq(sb->irq, sunix_interrupt, SA_SHIRQ, "snx", sb);
			...
			if (status)
			{
	 			printk("SNX Error: SUNIX Multi-I/O %s Board(bus:%d device:%d), request\n", sb->pb_info.board_name, sb->bus_number, sb->dev_number);
...
static int __init snx_init(void)
{
	...
	printk("\n\n");
	printk("=====================  SUNIX Device Driver Module Install  =====================\n");
	printk("\n");
	printk("SNX Info : Loading SUNIX Multi-I/O Board Driver Module\n");
	printk("                                                       -- Date :    %s\n", SNX_DRIVER_DATE);
	printk("                                                       -- Version : %s\n\n", SNX_DRIVER_VERSION);
...
step1_fail:
	...
	printk("SNX Error: Couldn't Loading SUNIX Multi-I/O Board Driver Module correctly,\n");
...
{
	printk("\n\n");
	printk("====================  SUNIX Device Driver Module Uninstall  ====================\n");
	printk("\n");
	...
	printk("SNX Info : Unload SUNIX Multi-I/O Board Driver Module Done.\n");
	printk("================================================================================\n");
}
