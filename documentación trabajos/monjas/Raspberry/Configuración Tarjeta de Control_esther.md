...
sudo apt-get purge libserialport* -y &&
...
## 1.12 Configuracion conversor serie a ethernet
...
Creamos un nuevo archivo en `/etc/systemd/system/serial-ethernet-converter.service` y escribimos lo siguiente:
...
SyslogIdentifier=serial-ethernet-converter

ExecStart=/usr/bin/socat tcp-listen:10001,reuseaddr,fork file:/dev/ttySERIAL,nonblock,cs8,b2400,cstopb=0,raw,echo=0
Restart=always
...
sudo systemctl enable serial-ethernet-converter &&
sudo systemctl start serial-ethernet-converter
```
...
# Keepalive
mv app.js keepalive.js
nano inicio.sh
   ```bash
#!/bin/bash
exec node /home/soa/keepalive/keepalive.js &>/dev/null
   ```
chmod +x inicio.sh  
sudo nano /etc/rc.local
 -> incluir antes del exit 0
 ```bash
 exec  /home/soa/keepalive/inicio.sh
 ```

 * si esta instalado el pm2
 sudo su
  pm2 list
  pm2 stop name
  pm2 delete name
 ...
 # Cuando conversor comprobaciones
ls -l /dev/tty*
sudo htop
socatac tpc-lisen 1001