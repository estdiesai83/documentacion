...
​	Cambiar teclado (opcion 3) selecionar Generic 105-key PC (intl), -> Other -> spanish
...
###### Instalación paquetes necesarios: vim, htop, glib, serialport, wiringpi
...
```
$ sudo cp /home/soa/rules/dali.rules /etc/udev/rules.d/
```
...
- Compilar el proyecto mostrando todos los mensajes por pantalla (ESTE SE HACIA ANTES)
...
```bash
$ sudo make prod (HACER SOLO ESTE)
```
...
2. En el proyecto comunicador antes de compilarlo hay que modificar en component.ts:
...
3. Compilar proyecto: sudo ng build --base-href=./ --prod

4. Copiar el contenido de la carpeta dist/videoportero/ en /*var*/www/html
...
##### Backup de la tarjeta SD de nuestra Raspberry Pi desde la terminal
...
[   <img src="file:///tmp/lu39249vqxrxe.tmp/lu39249vqxs29_tmp_368259a438202dba.png" alt="backup-raspberry-pi-os" style="zoom:33%;" /> ](http://fpaez.com/wp-content/uploads/2014/09/backup-raspberry-pi-os.png)

Para crear un backup de la tarjeta SD desde la terminal usaremos el comando **dd** que nos creará una copia idéntica de los archivos y dispositivos que indiquemos a un archivo ***.img** listo para ser regrabado. Tan solo debemos escribir estos 2 comandos en la terminal, desde un pc, no desde la raspberry:
...
Para sacar backup de la sd:

```bash
Ej: sudo umount /dev/sdb1  && sudo umount /dev/sdb2 
    sudo dd if=/dev/sdb of=/media/soa/backup_sd_camara.img bs=1M
```

Para copiar la imagen a un sd nueva:

```bash
Ej: sudo dd if=/media/soa/backup_sd_camara.img of=/dev/sdb bs=1M
```
...
Para conectarme a la placa por ejemplo

```
ssh 88.12.32.148 puerto 20134 pass: adminr123
```
...
Me pongo encima del proceso soaraspberry y pulso F9 y enter y solo se quita ese proceso
...
En el archivo soa.data están los valores que trae la raspberry