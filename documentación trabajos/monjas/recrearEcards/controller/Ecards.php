...
class Ecards extends CI_Controller {
   ...
   */
   ...
   /****************************GENERAR ESTRUCTURAS A PARTIR DE DATOS MORALEJA*****************/
   public function generarEstructurasDesdeMoraleja(){
      //crear directorio imagenes en tmp para importar las fotos
      system("rm -fr /tmp/imagenes");
      mkdir("/tmp/imagenes");

      //lee los valores de tabla "empresas"
      $post = $this->ecards->leerTablaEmpresas();         

      //por cada empresa crear las estructuras que corresponda   
      for($i=0;$i<count($post);$i++)
      {
         //por cada empresa
         $respuesta = $this->ecards->crearEmpresa($post[$i]->cif);

         if ( $respuesta == 0 ) {
            $clave=$this->ecards->crearCasoTcasoClaveEmpresaAccesoPorRegistro($post[$i]->cif,$post[$i]->empresa);
            $this->ecards->crearCasoTcasoEmpresa($post[$i]->cif,$post[$i]->empresa,$post[$i]->ciudad,$post[$i]->telefono,$post[$i]->email,$post[$i]->web);
            $this->ecards->crearCasoTcasoRepresentante($post[$i]->cif,$post[$i]->nombre,$post[$i]->apellidos,"",$post[$i]->cargo,"","","");
            //$this->ecards->crearCasoTcasoEntidad($post[$i]->cif,$post[$i]->entidades);                                                         
         }
      }
      
      //por cada usuario(master) leido coger los valores necesarios de datoscomplementarios y crear las estructuras:
      // tabla usuario: con tcaso master, tcaso complementario, 
      $post_obli = $this->ecards->leerDatosObligatoriosMaster();
      for($j=0;$j<count($post_obli);$j++)
      {
         //crea tabla usuario por cada caso leido
         $respuesta = $this->ecards->crearMaster($post_obli[$j]->nif);
         if ( $respuesta == 0 ) {
            //por cada caso leido, lee sus datos complementarios correspondientes
            $post_compl = $this->ecards->leerDatosComplementariosMaster($post_obli[$j]->codigosupermaster);

            //prepara valores leidos
            $direccion = isset($post_obli[$j]->direccion) ? $post_obli[$j]->direccion : '';
            $tlf = isset($post_compl[0]->tlf) ? $post_compl[0]->tlf : '';
            $email = isset($post_compl[0]->email) ? $post_compl[0]->email : '';
            $apellido2 = isset($post_obli[$j]->apellido2) ? $post_obli[$j]->apellido2 : '';
            $poblacion = isset($post_compl[0]->poblacion) ? $post_compl[0]->poblacion : '';
            $provincia = isset($post_compl[0]->provincia) ? $post_compl[0]->provincia : '';
            $codigopostal = isset($post_compl[0]->codigopostal) ? $post_compl[0]->codigopostal : '';
            $fechanacimiento = isset($post_compl[0]->fechanacimiento) ? $post_compl[0]->fechanacimiento : '';
            ...
            //crea tcaso master y complementarios en tabla usuario
            $tabla="usuario_".$post_obli[$j]->nif;
            $codigomaster=$this->ecards->crearDatosMaster($tabla, $post_obli[$j]->nombre, $apellidos, $tlf);
            $this->ecards->crearDatosComplementarios($tabla,$fechanacimiento,$direccion,$poblacion,$provincia,$codigopostal,$email,$codigomaster); 
            //crea clave
            $this->crearClaveMaster($post_obli[$j]->nif, $post_obli[$j]->nombre,$tlf,1);

            //importar foto y crear tcaso adjunto
            $foto=$this->ecards->leerFotoaImportarDatosComplementariosMaster($post_obli[$j]->codigosupermaster,$j);
            
            if(empty($foto))
            {
               $nombrefoto="foto".$j.".png";               
            
               $this->ecards->crearCasoTcasoAdjuntoMaster($tabla, $nombrefoto, $codigomaster);
            }
         }
      }
      
      //por usuario crea caso tcaso master y complementarios en las tablas empresa 
      //(la mitad en el corte ingles y la otra mitad en santander)
      for($j=0;$j<count($post_obli);$j++)
      {
         //por cada caso leido, lee sus datos complementarios correspondientes
         $post_compl = $this->ecards->leerDatosComplementariosMaster($post_obli[$j]->codigosupermaster);
         ...
         if($j<=count($post_obli)/2)
            $tabla="empresa_A39000013";//banco santander
         else
            $tabla="empresa_A28017895";//el corte ingles
         ...
         //crea tcaso empresa en tabla usuario
         $tabla="usuario_".$post_obli[$j]->nif;
         if($j<=count($post_obli)/2)
         {
            $cif="A39000013";//banco Santander
            $empresa="Banco Santander";//banco Santander
            $web="gruposantander.com";
         }               
         else   
         {
            $cif="A28017895";//el corte ingles
            $empresa="Grupo El Corte Inglés";
            $web="elcorteingles.es";
         }

         $this->ecards->crearCasoTcasoEmpresaMaster($tabla, $web, $empresa,$cif);
      }  
      echo "\n FIN CREAR TCASOS EMPRESA ";    
   }
}
