...
9. composer.lock: configuración de los paquetes que vamos a ir utilizando.

10. composer.json: configuración de los paquetes que vamos a ir utilizando.
...
A partir de la versión 11, Laravel eliminó varias carpetas que antes venían por defecto (como `app/Console`, `app/Exceptions` y `app/Http/Middleware`).

- **¿Dónde están ahora?** No es que hayan desaparecido del framework, sino que ahora están ocultas dentro del "núcleo" (`vendor`). Solo aparecen en tu carpeta `app` si realmente necesitas personalizarlas.
- **El archivo `routes/console.php`:** Ahora es el lugar principal para definir comandos de consola sencillos (tipo Closure), sustituyendo al antiguo `Kernel.php`.

------

## 2. Cómo hacer que aparezcan

Si necesitas crear un comando de consola personalizado o configurar tareas programadas (Scheduling), tienes dos opciones:

### A. Crear un comando manualmente

En cuanto ejecutes el comando de Artisan para crear un comando, Laravel **generará automáticamente** la carpeta por ti:
...
Tras esto, verás que aparece `app/Console/Commands/MiNuevoComando.php`.

### B. Publicar los archivos de configuración

Si lo que buscas es gestionar el **Scheduling** (programación de tareas) o los **Middlewares** como se hacía antes, ahora se centraliza en el archivo `bootstrap/app.php`.

Sin embargo, si prefieres "ver" las carpetas de nuevo para trabajar a la antigua usanza, puedes usar este comando:

Bash

```
php artisan install:api # (Opcional, si buscas controladores de API)
```

O simplemente configurar las excepciones y middlewares directamente en `bootstrap/app.php`:

------

## Comparativa de Estructura

| **Elemento**            | **Laravel 10 (y anteriores)** | **Laravel 11+**      |
| ----------------------- | ----------------------------- | -------------------- |
| **Comandos de Consola** | `app/Console/Kernel.php`      | `routes/console.php` |
| **Middlewares**         | `app/Http/Kernel.php`         | `bootstrap/app.php`  |
| **Excepciones**         | `app/Exceptions/Handler.php`  | `bootstrap/app.php`  |

------

### Un consejo de peer a peer:

No intentes crear las carpetas a mano. La filosofía de Laravel ahora es **"si no lo usas, no te lo muestro"**. Deja que los comandos de `php artisan make:...` construyan la estructura por ti según la vayas necesitando. Mantendrás el código mucho más limpio.



En laravel 11 en routes los ficheros de api.php y channels no me aparecen., para crearlos
...
A las rutas le podemos asociar un nombre, para ello utilizamos -> name
